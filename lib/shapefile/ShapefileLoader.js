class ShapefileLoader {
  constructor() {
    this.transform = null;
  }

  async load(path) {
    const matLine = new THREE.LineBasicMaterial({
      color: 0xff0000,
      linewidth: 3, // in pixels
      resolution: new THREE.Vector2(1000, 1000),
      dashed: false,
    });

    const features = await this.loadShapefileFeatures(path);
    const node = new THREE.Object3D();

    for (const feature of features) {
      const fnode = this.featureToSceneNode(feature, matLine);
      node.add(fnode);
    }

    let setResolution = (x, y) => {
      matLine.resolution.set(x, y);
    };

    const result = {
      features: features,
      node: node,
      setResolution: setResolution,
    };

    return result;
  }

  featureToSceneNode(feature, matLine) {
    let geometry = feature.geometry;

    let color = new THREE.Color(1, 1, 1);

    let transform = this.transform;
    if (transform === null) {
      transform = { forward: (v) => v };
    }
    console.log(feature.geometry);
    if (feature.geometry.type === 'PointZ') {
      let coordinates = [];

      coordinates.push(
        new THREE.Vector3(
          geometry.coordinates[0],
          geometry.coordinates[1],
          geometry.coordinates[2]
        )

      );
     // console.log("coord 0 = "+ geometry.coordinates[0]);
      // console.log("coordinates 2 = " + geometry.coordinates[2]);


      // let long = geometry.coordinates[0];
      // let lat = geometry.coordinates[1];
      // let height = geometry.coordinates[2];
      //console.log(height)

      var particle_geom = new THREE.Geometry().setFromPoints(coordinates);
      //particle_geom.vertices.push(new THREE.Vector3(long, lat, height));

      var particle_material = new THREE.PointsMaterial({color: 0x80FFFF});

      var particle = new THREE.Points(particle_geom, particle_material);
      return particle;


      // let sg = new THREE.SphereGeometry(1, 18, 18);
      // let sm = new THREE.MeshNormalMaterial();
      // let s = new THREE.Mesh(sg, sm);

      // //let [long, lat, height] = geometry.coordinates;

      // let long = geometry.coordinates[0];
      // let lat = geometry.coordinates[1];
      // let hei = geometry.coordinates[2];

      // //let pos = transform.forward([long, lat]);

      // s.position.set(long,lat,hei);

      // s.scale.set(10, 10, 10);

      // return s;



    } else if (geometry.type === 'LineStringZ') {
      let coordinates = [];

      let min = new THREE.Vector3(Infinity, Infinity, Infinity);
      for (let i = 0; i < geometry.coordinates.length; i++) {
        // let [long, lat] = geometry.coordinates[i];
        // let pos = transform.forward([long, lat]);

        // min.x = Math.min(min.x, pos[0]);
        // min.y = Math.min(min.y, pos[1]);
        // min.z = Math.min(min.z, 20);

        coordinates.push(
          new THREE.Vector3(
            geometry.coordinates[i][0],
            geometry.coordinates[i][1],
            geometry.coordinates[i][2]
          )
        );

        // if (i > 0 && i < geometry.coordinates.length - 1) {
        //   coordinates.push(...pos, 20);
        // }
      }

      //   for (let i = 0; i < coordinates.length; i += 3) {
      //     coordinates[i + 0] -= min.x;
      //     coordinates[i + 1] -= min.y;
      //     coordinates[i + 2] -= min.z;
      //   }

      const lineGeometry = new THREE.BufferGeometry().setFromPoints(
        coordinates
      );

      //   lineGeometry.setPositions(coordinates);

      const line = new THREE.Line(lineGeometry, matLine);

      //   line.distance();
      //   line.scale.set(1, 1, 1);
      //   line.position.copy(min);

      return line;
      // } else if (geometry.type === 'Polygon') {
      //   for (let pc of geometry.coordinates) {
      //     let coordinates = [];

      //     let min = new THREE.Vector3(Infinity, Infinity, Infinity);
      //     for (let i = 0; i < pc.length; i++) {
      //       let [long, lat] = pc[i];
      //       let pos = transform.forward([long, lat]);

      //       min.x = Math.min(min.x, pos[0]);
      //       min.y = Math.min(min.y, pos[1]);
      //       min.z = Math.min(min.z, 20);

      //       coordinates.push(...pos, 20);
      //       if (i > 0 && i < pc.length - 1) {
      //         coordinates.push(...pos, 20);
      //       }
      //     }

      //     for (let i = 0; i < coordinates.length; i += 3) {
      //       coordinates[i + 0] -= min.x;
      //       coordinates[i + 1] -= min.y;
      //       coordinates[i + 2] -= min.z;
      //     }

      //     const lineGeometry = new THREE.BufferGeometry().setFromPoints(
      //       coordinates
      //     );
      //     // lineGeometry.setPositions(coordinates);

      //     const line = new THREE.Line3(lineGeometry, matLine);
      //     // line.distance();
      //     line.scale.set(1, 1, 1);
      //     line.position.copy(min);

      //     return line;
      //   }
    } else {
      //console.log('unhandled feature: ', feature);
    }
  }

  async loadShapefileFeatures(file) {
    let features = [];

    let source = await shapefile.open(file);

    while (true) {
      let result = await source.read();

      if (result.done) {
        break;
      }

      if (
        result.value &&
        result.value.type === 'Feature' &&
        result.value.geometry !== undefined
      ) {
        //console.log("results value is a feature");
        //console.log(result.value.geometry);
        features.push(result.value);
      }
    }

    return features;
  }
}
