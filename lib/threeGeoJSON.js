...
function drawThreeGeo(json, radius, shape, materalOptions, container) {
    ...
    function getConversionFunctionName(shape) {
        ...
        //Added a new shape 'Grid' to handle the British National Grid coordinates used in FlightLines.geojson
        else if (shape == 'Grid') {
            conversionFunctionName = convertToGridCoords;
        } 
        //Added a new shape 'Coord' to handle BNG without normalizing
        else if (shape == 'Coord') {
            conversionFunctionName = convertToCoords;
        }
        else {
    ...
    function convertToPlaneCoords(coordinates_array, radius) {
        ...
        //Added z_values as zero in order to work
        z_values.push(0);
    }


    //New function to normalise BNG coordinates
    function convertToGridCoords(coordinates_array, radius) {
        var xCoord = coordinates_array[0];
        var yCoord = coordinates_array[1];
        var height = coordinates_array[2];

        y_values.push((yCoord / 2500000) * radius);
        x_values.push((xCoord / 2500000) * radius);
        z_values.push((height / 2500000) * radius);
    }


    //New function to load BNG coordinates without normalising 
    function convertToCoords(coordinates_array, radius) {
        var xCoord = coordinates_array[0];
        var yCoord = coordinates_array[1];
        var height = coordinates_array[2];

        y_values.push(yCoord);
        x_values.push(xCoord);
        z_values.push(height);
    }

    //'THREE.ParticleSystem' has been changed to 'THREE.Points' in THREE.js now 
    //See 'https://github.com/mrdoob/three.js/issues/4065' 
